    ...
    <div class="wrapper">
        <div class="container-fluid">
            <div class="row">
                <div class="col-md-12">
                    <div class="mt-5 mb-3 clearfix">
                        <h2 class="pull-left">Approved Regions</h2>
                    </div>
                    <?php
                    // Include config file
                    require_once "../DBConnection.php";                    
                    // Attempt select query execution
                    $sq = "select c.Description_Place, c.Floor, e.Description_Place, e.Floor from ActiveCustomerLiveToRooms as a join ApprovedEntries as b on b.BraceletId=a.BraceletId join EntriesToRegions as d on d.idEntriesToRegions=b.idEntriesToRegions join Regions as c on c.idRegions=d.idRegionsTo join Regions as e on e.idRegions=d.idRegionsFrom where a.BraceletId='$bid' ";
                    if($result = mysqli_query($con, $sq)){
                        if(mysqli_num_rows($result) > 0){
                            echo '<table class="table table-bordered table-striped">';
                                echo "<thead>";
                                    echo "<tr>";
                                        echo "<th>From Region</th>";
                                        echo "<th>Of Floor</th>";
                                        echo "<th>To Region</th>";
                                        echo "<th>Of Floor</th>";
                                    echo "</tr>";
                                echo "</thead>";
                                echo "<tbody>";
